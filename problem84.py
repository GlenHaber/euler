...
def simulate_monopoly(sides):
    ...
    go_to_jail = board.index('G2J')
    railroads = [board.index('R' + x) for x in '1234']
    utilities = [board.index('U' + x) for x in '12']

    space = 0
    visits = [0 for _ in board]
    community_chest = [go, jail] + [None] * 14
    chance = [go, jail, board.index('C1'), board.index('E3'), board.index('H2'),
              board.index('R1'), railroads, railroads, utilities, -3] + [None] * 6
    ...
    def handle(card):
        if card is None:
            return space
        elif isinstance(card, int):  # Go to specific space
            if card < 0:  # Go back 3
                return space + card
            else:  # All other "go to space" cards
                return card
        else:  # List: Go to next of these spaces
            if space > max(card):
                return min(card)
            else:
                return min(filter(lambda c: c > space, card))

    for _ in range(10000000):
        space = (space + roll()) % len(board)
        if space == go_to_jail:
            space = jail
        if board[space].startswith('CC'):
            cc_card = community_chest.pop()
            space = handle(cc_card)
            community_chest.insert(0, cc_card)
        elif board[space].startswith('CH'):
            ch_card = chance.pop()
            space = handle(ch_card)
            chance.insert(0, ch_card)
        visits[space] += 1

    popular = sorted(range(40), key=lambda i: visits[i], reverse=True)
    return '{:02}{:02}{:02}'.format(*popular[:3])
