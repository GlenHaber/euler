...
def abundant_numbers(ceiling=None):
    n = 1
    while True:
        n += 1
        if is_abundant(n):
...
abundant = {a for a in abundant_numbers(28123)}
abundant_sums = {a + b for a, b in itertools.combinations_with_replacement(abundant, 2)}

print(sum(i for i in range(28124) if i not in abundant_sums))
