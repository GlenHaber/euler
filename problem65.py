...
def continued_fraction_e(max_recursions):
    coeffs = [2]
    k = 0
    while len(coeffs) < max_recursions:
        k += 1
        coeffs.extend([1, 2 * k, 1])
    return coeffs[:max_recursions]


def eval_continued_fraction(coeffs):
    n = coeffs[-1]
    d = 1
    for c in coeffs[-2::-1]:
        n, d = d + n * c, n
...
assert digital_sum(eval_continued_fraction(continued_fraction_e(10))[0]) == 17
print(digital_sum(eval_continued_fraction(continued_fraction_e(100))[0]))
