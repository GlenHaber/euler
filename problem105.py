...
{157, 150, 164, 119, 79, 159, 161, 139, 158} satisfies both rules for all possible subset pair combinations and
S(A) = 1286.

Using sets.txt (right click and "Save Link/Target As..."), a 4K text file with one-hundred sets containing seven to
...
from problem103 import is_special, S

start = time()
sets = [set(map(int, set_.rstrip().split(','))) for set_ in open('p105_sets.txt')]
specials = [s for s in sets if is_special(s)]
print(len(specials))
print(sum(S(set_) for set_ in specials))
print(time() - start)
